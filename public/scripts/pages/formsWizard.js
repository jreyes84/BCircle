...
var FormsWizard = function() {
    ...
    return {
        init: function() {
            ...
            $('#progress-wizard-pasivo').formwizard({focusFirstInput: true, disableUIStyles: true, inDuration: 0, outDuration: 0});

            // Get the progress bar and change its width when a step is shown
            var progressBar = $('#progress-bar-wizard');
            var progressBarPasivo = $('#progress-bar-wizard-pasivo');
            ...
            $("#progress-wizard-pasivo").bind('step_shown', function(event,data){
                if (data.currentStep === 'progress-first-pasivo') {
                    progressBarPasivo
                        .css('width', '33%')
                        .attr('aria-valuenow', '33')
                        .removeClass('progress-bar-warning progress-bar-success')
                ...
                else if (data.currentStep === 'progress-second-pasivo') {
                    progressBarPasivo
                        .css('width', '66%')
                        .attr('aria-valuenow', '66')
                        .removeClass('progress-bar-danger progress-bar-success')
                ...
                else if (data.currentStep === 'progress-third-pasivo') {
                    progressBarPasivo
                        .css('width', '100%')
                        .attr('aria-valuenow', '100')
                        .removeClass('progress-bar-danger progress-bar-warning')
